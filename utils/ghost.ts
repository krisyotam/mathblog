...
export async function getPosts(): Promise<GhostPost[]> {
  try {
    ...
    const filteredPosts = data.posts.filter((post: GhostPost) => {
      const hasKrisPureMath = post.tags.some((tag: { name: string }) => tag.name === '#krispuremath');
      const hasExcludedTags = post.tags.some((tag: { name: string }) =>
    ...
    // Parse front matter
    return filteredPosts.map((post: GhostPost) => {
