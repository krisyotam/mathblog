...
function convertMarkdownToHTML() {
    fs.readdir(markdownDirectory, (err, files) => {
        ...
        markdownFiles.forEach(file => {
            ...
            const markdownStats = fs.statSync(markdownFilePath);
            let htmlExists = fs.existsSync(htmlFilePath);
            let htmlStats = htmlExists ? fs.statSync(htmlFilePath) : null;

            // Only regenerate the HTML if the Markdown is newer or if the HTML doesn't exist
            if (!htmlExists || markdownStats.mtime > htmlStats.mtime) {
                fs.readFile(markdownFilePath, 'utf8', (err, data) => {
                    ...
                    fs.writeFile(htmlFilePath, htmlPage, (err) => {
                        ...
                        console.log(`HTML file created/updated: ${file.replace('.md', '.html')}`);
                    });
                });
            }
        ...
        fs.readdir(htmlDirectory, (err, htmlFiles) => {
            ...
            htmlFiles.forEach(htmlFile => {
                ...
                // Delete HTML file if it doesn't have a corresponding markdown file
